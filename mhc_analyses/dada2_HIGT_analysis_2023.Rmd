---
title: "MHC1 Analysis in Hawaiian Green Turtles with DADA2- trimmed primers and conservative filtering, additional Data"
...
* This analysis is being redone in 2023 with 55 additional samples, some of which have tumor scores. IMPORTANT- these 55 samples had fewer miseq cycles (uses nano 300 cycle kit) so the PE reads were shorter than the first run. Because of this, there is no primer contamination to trim off in dada2 after using sickle/scythe. So I will use the files that were already filtered and trimmed from the previous run (those that were output into the "filtered" folder) as well as the new data that has been put through sickle and scythe 
...
This is error rates with just the NEW data
...
```
```{r echo=FALSE, warning=FALSE, message=FALSE}
## look at errors using ALL samples (old and new sequence data)
path <-"./MHC_HI_fastq/filtered" #change here to use old samples also
...
# Extract sample names, assuming filenames have format: SAMPLENAME_XXX.fastq
sample.names <- sapply(strsplit(basename(fnFs), "_F_filt"), `[`, 1) #set delimiter to be _R instead of just _ so that I get all sample info in name  up to the R1/R2 designation
filtFs <- file.path(path, paste0(sample.names, "_F_filt.fastq.gz"))
filtRs <- file.path(path, paste0(sample.names, "_R_filt.fastq.gz"))
names(filtFs) <-sample.names
names(filtRs) <-sample.names
#learn error rates. This takes about 4 minutes to run, especially on windows without multithread
errF <- learnErrors(filtFs, multithread=FALSE) #100094844 total bases in 702059 reads from 79 samples will be used for learning the error rates. dada2 only uses a subset of samples by default
errR <- learnErrors(filtRs, multithread=FALSE) #100464493 total bases in 694932 reads from 77 samples will be used for learning the error rates.
plotErrors(errF, nominalQ=TRUE)#plot error rates for each possible nucleotide transition based on quality score. error rates should drop with increased quality, and black line should be good fit to observed points
plotErrors(errR, nominalQ = TRUE)

```
This is error rates with ALL data. This generally fits better, so I'll continue with using these error rates.
...
dadaFs[[1]] #35 sequence variants were inferred from 751 input unique sequences.
#Key parameters: OMEGA_A = 1e-40, OMEGA_C = 1e-40, BAND_SIZE = 16
#above inspection is only for ONE sample- so first sample has 35 true sequence variants from 2020 input sequences
```
...
#dim(seqtab) #dim are 90 x 742, so 90 samples by 742 sequences
```

There are 90 samples and a total 742 sequence variants at this point. These sequence variants will be filtered further in the following steps.
...
#lenghts vary between 75-195
seqtab2<-seqtab[,nchar(colnames(seqtab)) %in% 162]
#table(nchar(getSequences(seqtab2))) #confirmed, now only has 676 variants with all same length of 190 bp
```
...
#dim(seqtab.nochim) #leaves all 90 samples but only 133 variants remain, which is still a lot. if MOST were removed, might need to change upstream filters
sum(seqtab.nochim)/sum(seqtab2) #0.8626798


#write.table(seqtab.nochim, "mhc_unfilt_variants.csv",sep=",",row.names = TRUE, col.names = TRUE)

```
 86.3% of sequences were retained as non-chimeric sequences, but many unique sequence variants are removed at this step- only 133 remain from 676
...
The output table has a row for each sample, and displays the number of reads retained at each point in the pipeline above. For this dataset, I am seeing the most loss at the filtering step, and some loss at the merging step as well. 

Sample 14 clearly failed on merging, and will be filtered out below, as well as MHC_G216-s6_Oahu_NA_1 (visual inspection of this sample showed only very short reads in fastq file)
...
mod_seqtable<-filter(mod_seqtable,sums>1) #keep only variants that are present in more than 1 sample, leaves 78 variants
...
reads_by_samp<-as.data.frame(sum_reads_by_samp[c(1:90)]) #put reads per sample in a dataframe with sample names
  
failed_samples<-ifelse (reads_by_samp<1000,"fail","pass") #consider failed samples as those than have fewer than 1k reads, if adjusted to 400 reads this would keep 5 more samples, but not doing this yet
...
sum(failed_samples$`sum_reads_by_samp[c(1:90)]`=="fail") #how many samples failed? 11 here, 10 from the new dataset
failed_samples<-filter(failed_samples, grepl("fail",`sum_reads_by_samp[c(1:90)]`)) #get list of failed samples
failed_samp_names<-row.names(failed_samples)  
#remove failed samples
mod_seqtable<-mod_seqtable[, !(names(mod_seqtable) %in% failed_samp_names)] #remove columns for samples that failed 



```

At this point, 79/90 individuals remain in the dataset. 

#### v. Filter out variants calls in individuals with less than 100 reads

I then removed variant assignments in individuals where there were fewer than 100 reads supporting that assignment. For individuals with 5000 total reads, that would mean removing variant assignments that comprised less than 2% of total reads. This is similar to filtering done in Stervander et al. where assignments were removed if the supporting reads were less than 5% of total reads for an individual. However this would mean that for samples with 1000 reads I would be removing 10% of the reads
...
mod_seqtable$sums<- apply(mod_seqtable,1, function(i) sum(i>0)) #add column to dataframe showing how many samples have this variant
mod_seqtable<-filter(mod_seqtable,sums>2) #same as above, redoing now that more cells have a 0 value. also have extra col with value that is counted in sum, so putting 2 here is same as 1 above.Now have 34 variants
sum_reads_by_samp_filt<-apply(mod_seqtable,2,function(i) sum(i)) #count total number of reads per sample remaining
reads_by_samp_filt<-as.data.frame(sum_reads_by_samp_filt[c(1:81)]) #check to see if any more samples have totally failed and need to be removed- in previous iteration with og data, MHC_Sample21_cm_Oahu had failed after this level of filtering, but now it is being retained
...
#same as above, but dividing each sum of individuals with a variant by total number of individuals, 79 at this point
mod_seqtable$rel_indiv<-mod_seqtable$sums/79
...
From this, we see that there are 10 variants that fall below a 5% threshold given the total number of individuals in this population. If we were to remove these 10 variants, we would be left with 24 variants.
...
freq_matrix2<-t(freq_matrix[,1:79]) #transpose freq matrix and only keep sample columns
...
freq_matrix<-freq_matrix[c(1:79)]#remove last 3 columns that had sums/old frequency calcs
...
mod_seqtable2<-mod_seqtable2[c(2:80)]#get rid of old sums and old row name column
...
#check again to make sure there aren't any failed samples after this filter
sum_reads_by_samp_filt<-apply(mod_seqtable2,2,function(i) sum(i)) #count total number of reads per sample remaining
reads_by_samp_filt<-as.data.frame(sum_reads_by_samp_filt[c(1:81)]) #check to see if any more samples have totally failed and need to be removed- in previous iteration with og data, MHC_Sample21_cm_Oahu had failed after this level of filtering, but now it is being retained

write.csv(mod_seqtable2,"mhc_filteredvars_5.30.23.csv") #export final variant table
...
#get column of frequency in dataset for each variant- this is frequency by TOTAL READS IN DATASET NOT INDIVIDUAL
#total_reads<-sum(mod_seqtable2$total_calls) 
#mod_seqtable2$variant_freq<-mod_seqtable2$total_calls/total_reads*100


#ggplot(mod_seqtable2,aes(variant_freq))+
  geom_density()

#mean(mod_seqtable2$variant_freq) #4.34
#min(mod_seqtable2$variant_freq) #0.0789
#max(mod_seqtable2$variant_freq) #12.99
#median(mod_seqtable2$variant_freq) #3.346
```
...
no_alleles<-no_alleles[c(1:79)] #only keep allele numbers for first 34 columns which are the samples
no_alleles<-as.data.frame(no_alleles)

min(no_alleles$no_alleles) #2
max(no_alleles$no_alleles) #7
mean(no_alleles$no_alleles)#4.303

#frequency of alleles occuring in all individuals
mod_seqtable2$indfreq<-mod_seqtable2$sums/79
ggplot(mod_seqtable2,aes(indfreq))+
  geom_density()

```
This is very close to previous results, with max and min both one higher
...
mod_seqtable2<-read.csv("2023_outputs/mhc_filteredvars_5.30.23.csv")
rownames(mod_seqtable2)<-mod_seqtable2$Sequence 
mod_seqtable2<-mod_seqtable2[c(3:81)]


#transpose filtered table so that it meets input requirements
mod_seqtable2<-mod_seqtable2[c(1:79)]#keep only columns with sample names
...
Then I looked at density of mean distances per individual. On average, individuals had a mean p-distance of 0.2519 (up from 0.24 with last dataset), indicating most indivduals had fairly low sequence variation. However, the maximum was 0.3642 and the minimum was 0.055, so there are at least some individuals with slightly higher sequence diversity as well as some with very low sequence diversity. 



```{r echo=FALSE, warning=FALSE, message=FALSE}
#plot sequence similarity
dist_matrix<-read.csv("./2023_outputs/dist_matrix_20230530.csv")
indiv_meanp<-read.csv("./2023_outputs/mean_dist_table_20230530.csv")
...
CreateFas(matrix_for_pdist,"./2023_outputs") #make fasta file with MHC sequences, filename has date 
...
- the overall diversity of position score was 81.1%. This was done by comparing all sequences in the dataset to the first sequence in the dataset
...
#look at diversity using ORF3:
aa_scores_orf3<-read.table("./2023_outputs/orf3_scorecons.txt")
...
Based on this figure, 9 amino acid positions are variable at levels of 0.5 or greater, indicating that at 9 out of 53 amino acids the sequences are over 50% variable.
...
library(ShortRead) ####not avail for this version of R 4.3?
library(pegas)
library(ape)
...
abund_samp_tab<-mod_seqtable2[c(1,81)] #extract sequence and value from column
...
writeFasta (seqs.re_replicated,file="./2023_outputs/derepseqsbyindiv.fasta") #write all dereplicated sequences to fasta file for network analysis
...
#NB I exported the fasta in an order that corresponded to the order of sequence names I gave when I previously ran the small dataset, so var I in this dataset is also variant I in the last dataset
chmy_seqs #all seqs 162bp, stored in matrix, gets base composition for each base
...
sz<-sz[chmynet.labs]
plot(chmynet,size=sz) #plots bubble size based on number of individuals who have this haplotype

#set more options:
setHaploNetOptions(haplotype.inner.color = "#CCCC4D",haplotype.outer.color = "#CCCC4D",show.mutation = 3, labels = TRUE, scale.ratio=1,link.type=1)
tiff("MHCnetworkbyindiv.tiff", width=500, height=500)
plot(chmynet,size=sz)
dev.off()
